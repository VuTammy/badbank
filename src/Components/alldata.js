...
import UpdateData from "./userinfo";
...
function AllData() {
  ...
  function CreateData() {
    ...
    for (let i = 0; i < UpdateData.createName.length; i++) {
      var row = table.insertRow(1);
      var nameData = row.insertCell(0);
      var emailData = row.insertCell(1);
      var passwordData = row.insertCell(2);
      nameData.innerHTML = UpdateData.createName[i];
      emailData.innerHTML = UpdateData.createEmail[i];
      passwordData.innerHTML = UpdateData.createPassword[i];
    }
